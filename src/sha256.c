...
#define EP0(x) (ROTATE_RIGHT((x), 2u) ^ ROTATE_RIGHT((x), 13u) ^ \
    ROTATE_RIGHT((x), 22u))
#define EP1(x) (ROTATE_RIGHT((x), 6u) ^ ROTATE_RIGHT((x), 11u) ^ \
    ROTATE_RIGHT((x), 25u))
...
static void transform(Sha256Context *context, const uint8_t data[64]) {
    ...
    for (; i < 64; ++i) {
        words[i] = SIG1(words[i - 2]) + words[i - 7] +
            SIG0(words[i - 15]) + words[i - 16];
    }
