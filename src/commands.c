...
static CupError check_install_presence(const CupState *state, const char *component, const EntryContext *ctx, int *in_state, int *on_disk) {
    CupError err;

    if (state == NULL || component == NULL || ctx == NULL || 
        in_state == NULL || component[0] == '\0' || on_disk == NULL) {
        return CUP_ERR_INVALID_INPUT;
    }
...
static CupError perform_install(const char *tmp_path, const char *component, const char *tool, const char *release, const char *archive_format) {
    CupError err;
    char package_path[MAX_PATH_LEN];

    if (tmp_path == NULL || component == NULL || tool == NULL || release == NULL || archive_format == NULL ||
        tmp_path[0] == '\0' || component[0] == '\0' || tool[0] == '\0' || release[0] == '\0' || archive_format[0] == '\0'){
        return CUP_ERR_INVALID_INPUT;
    }
    ...
    err = write_component_info_at_path(tmp_path, component, tool, release);
    if (err != CUP_OK) {
        return err;
    }
...
static CupError rollback_committed_install(const char *component, const EntryContext *ctx) {
    CupError err;
    char install_path[MAX_PATH_LEN];
    char tmp_path[MAX_PATH_LEN];

    if (component == NULL || ctx == NULL || component[0] == '\0') {
        return CUP_ERR_INVALID_INPUT;
    }

    err = build_install_path(install_path, sizeof(install_path), component, ctx->tool, ctx->resolved_release);
    if (err != CUP_OK) {
        return err;
    }
...
CupError handle_list(void) {
    ...
    for (i = 0; i < state.installed_count; ++i) {
        ...
        err = resolve_entry_context(state.installed[i].component, state.installed[i].entry, &ctx);
        if (err != CUP_OK) {
        ...
        err = installation_exists(state.installed[i].component, ctx.tool, ctx.resolved_release, &on_disk);
        if (err != CUP_OK) {
        ...
        err = is_stable_release(state.installed[i].component, ctx.tool, ctx.resolved_release, &is_stable);
        if (err != CUP_OK) {
            is_stable = 0;
        }
...
CupError handle_install(const char *component, const char *entry, const char *format_override) {
    ...
    err = check_install_presence(&state, component, &ctx, &in_state, &on_disk);
    if (err != CUP_OK) {
        return err;
    }
    ...
    if (err != CUP_OK) {
        rollback_err = rollback_committed_install(component, &ctx);
...
CupError handle_remove(const char *component, const char *entry) {
    ...
    err = state_remove_default_if_matches(&state, component, ctx.canonical_entry);
    if (err != CUP_OK) {
        return err;
    }
...
CupError handle_current(const char *component) {
    ...
    err = resolve_entry_context(component, default_entry, &ctx);
    if (err != CUP_OK) {
    ...
    err = is_stable_release(component, ctx.tool, ctx.resolved_release, &is_stable);
    if (err != CUP_OK) {
        is_stable = 0;
    }

    if (is_stable) {
        printf("Current %s default: %s (stable)\n", component, default_entry);
    } else {
