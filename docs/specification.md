...
The release cycle is deliberate and driven only by `VERSION`: update `VERSION`, finish the development commits, and push them to `main`. `build-release.yml` starts on that push, reads `VERSION`, records whether tag `vX.Y.Z` already exists, and creates static release candidate assets only when a new release is needed. `test-release.yml` then runs automatically from that build run, exits cleanly for no-op release decisions, or tests the exact candidate artifacts and only after successful native tests creates tag `vX.Y.Z` and publishes the complete release. The same test workflow can also be run manually in source mode or against a selected build run id. There is no commit-count or automatic patch increment. `BUILD_MODE=release` controls optimization only; it does not by itself turn an arbitrary commit into an official release.
