...
The release cycle is explicit and uses `VERSION` as the only version input. `build-release.yml` starts automatically on pushes to `main` and can also be started manually. It reads `VERSION`, checks the corresponding `vX.Y.Z` tag, stores a release decision artifact, and builds static release candidate assets only when that tag does not already exist. `test-release.yml` is the only test workflow: it can run source tests manually, or it can test a release candidate automatically after `build-release.yml` completes or manually from a selected build run id. If the release decision says the tag already exists, it exits cleanly; otherwise it publishes the GitHub Release only after the candidate assets pass the native release tests.
