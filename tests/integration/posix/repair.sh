...
invalid_package=$TEST_HOME/.cup/components/debugger/lldb/$TEST_PLATFORM/$TEST_PLATFORM/23.1.0
...
temporary_name=install-compiler-clang-$TEST_PLATFORM-$TEST_PLATFORM-23.1.0-test
JOURNAL
...
foreign_tree=$TEST_HOME/.cup/components/compiler/clang/$foreign_host/$foreign_host/23.1.0
mkdir -p "$foreign_tree"
chmod u+w "$state_file"
printf 'installed.compiler.%s.%s=clang@23.1.0\n' \
...
assert_contains "$(cat "$state_file")" \
    "installed.compiler.$foreign_host.$foreign_host=clang@23.1.0"
