/*
 * Exercises asset inspection, checksum verification and platform-name helpers through
 * controlled filesystem/system boundaries.
 */
...
static void test_inspection_errors(void) {
    ...
    TEST_ASSERT_EQUAL_INT(CUP_ERR_TEMPORARY, assets_inspect(&inspection));
...
static void test_platform_asset_names(void) {
    ...
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_OS,
                          assets_platform_checksums_name(name, sizeof(name)));

    reset_scenario();
    TEST_ASSERT_EQUAL_INT(CUP_ERR_BUFFER_TOO_SMALL, assets_binary_asset_name(name, 2));
    TEST_ASSERT_EQUAL_INT(CUP_ERR_BUFFER_TOO_SMALL, assets_platform_checksums_name(name, 2));
}
