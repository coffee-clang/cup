/*
 * Test focus: Exercises SHA-256 vectors, file hashing and strict SHA256SUMS record selection.
 */
...
static void test_checksum_records(void) {
    ...
    char digest[SHA256_HEX_LENGTH + 1];
    char uppercase[SHA256_HEX_LENGTH + 1];
    int matches;
    FILE *file;
    size_t i;
...
static void test_checksum_validation(void) {
    ...
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_sha256_file(asset, digest, SHA256_HEX_LENGTH));
    TEST_ASSERT_EQUAL_INT(CUP_ERR_FILESYSTEM,
                          checksum_sha256_file(missing, digest, sizeof(digest)));

    /* Record lookup validates both the checksum file and the requested asset name. */
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_find_expected(NULL, "asset.bin", digest, sizeof(digest)));
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_find_expected(sums, "../asset.bin", digest, sizeof(digest)));
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_find_expected(sums, "asset.bin", NULL, sizeof(digest)));
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_find_expected(sums, "asset.bin", digest, SHA256_HEX_LENGTH));
    TEST_ASSERT_EQUAL_INT(CUP_ERR_FILESYSTEM,
                          checksum_find_expected(missing, "asset.bin", digest, sizeof(digest)));
    ...
    count = 99;
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT, checksum_validate_file(NULL, &count));
    TEST_ASSERT_EQUAL_size_t(0, count);
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT, checksum_validate_file(sums, NULL));
    TEST_ASSERT_EQUAL_INT(CUP_ERR_FILESYSTEM, checksum_validate_file(missing, &count));

    write_bytes(sums, "", 0);
    TEST_ASSERT_EQUAL_INT(CUP_ERR_VALIDATION, checksum_validate_file(sums, &count));

    /* A canonical checksum set accepts both binary and text separators. */
    TEST_ASSERT_EQUAL_INT(CUP_OK, checksum_sha256_file(asset, digest, sizeof(digest)));
    file = fopen(sums, "w");
    TEST_ASSERT_NOT_NULL(file);
    fprintf(file, "%s *asset.bin\n", digest);
    fprintf(file, "%s\tother.bin\n", digest);
    TEST_ASSERT_EQUAL_INT(0, fclose(file));
    TEST_ASSERT_EQUAL_INT(CUP_OK, checksum_validate_file(sums, &count));
