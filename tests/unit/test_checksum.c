/* Exercises canonical SHA-256 validation and hashing boundaries. */
...
int interrupt_requested(void) {
    if (interrupt_after_calls < 0) return 0;
...
static void test_hash_output_contracts(void) {
    ...
    TEST_ASSERT_EQUAL_STRING("", digest);
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_sha256_bytes(data, sizeof(data), NULL, sizeof(digest)));
    TEST_ASSERT_EQUAL_INT(CUP_OK,
                          checksum_sha256_bytes(data, sizeof(data), digest, sizeof(digest)));
    ...
    TEST_ASSERT_EQUAL_STRING("", digest);
    TEST_ASSERT_EQUAL_INT(CUP_OK,
                          checksum_sha256_stream(file, digest, sizeof(digest)));
    TEST_ASSERT_EQUAL_INT(0, (int)ftell(file));
    TEST_ASSERT_EQUAL_INT(0, fclose(file));

    strcpy(digest, "stale");
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_sha256_stream(NULL, digest, sizeof(digest)));
    TEST_ASSERT_EQUAL_STRING("", digest);
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_sha256_file(NULL, digest, sizeof(digest)));
    TEST_ASSERT_EQUAL_STRING("", digest);
    TEST_ASSERT_EQUAL_INT(CUP_ERR_INVALID_INPUT,
                          checksum_sha256_file("", digest, sizeof(digest)));
    TEST_ASSERT_EQUAL_INT(CUP_ERR_BUFFER_TOO_SMALL,
...
static void test_hashing_honors_interrupts(void) {
    ...
    for (i = 0; i < sizeof(blocks); ++i) blocks[i] = (unsigned char)(i * 17u + 3u);
...
int main(void) {
    TEST_ASSERT_NOT_NULL(test_make_temp_directory(temp_dir, sizeof(temp_dir), "cup-checksum-test"));
    UNITY_BEGIN();
